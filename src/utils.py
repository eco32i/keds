...
def split_by_index(read1, read2, barcodes, bc_pos=(26,6)):
    ...
    cnt = 0
    assigned = 0
    for rec1, rec2 in zip(parse_fastq(read1), parse_fastq(read2)):
        id1, seq1, q1 = rec1
        id2, seq2, q2 = rec2
        cnt += 1
        if cnt % 1000000 == 0:
        ...
        if ind in barcodes:
            ...
            if not(ind in output_files):
                r1 = gzip.open('../data/%s_R1.fastq.gz' % ind, 'wt')
                r2 = gzip.open('../data/%s_R2.fastq.gz' % ind, 'wt')
...
def split_pools(barcode, dirname='../data'):
    ...
    and `minus` pools.
    '''
    d, _, filenames = next(os.walk(dirname))
    ...
    for file_R1,file_R2 in zip(files_R1, files_R2):
        ...
        cnt_plus = 0
        cnt_minus = 0
        with gzip.open(os.path.join(dirname, barcode+'_minus.fastq.gz'), 'wt') as gz_minus, \
             gzip.open(os.path.join(dirname, barcode+'_plus.fastq.gz'), 'wt') as gz_plus:
            for rec1,rec2 in zip(parse_fastq(file_R1), parse_fastq(file_R2)):
                cnt += 1
                if cnt % 1000000 == 0:
            ...
            sys.stdout.flush()
    return {'barcode': barcode, 'plus': cnt_plus, 'minus': cnt_minus}
